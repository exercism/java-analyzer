...
public class GigasecondTest {
    ...
    public void testNullDate() {
        Gigasecond gigasecond = new Gigasecond((LocalDate) null);
        assertEquals(LocalDate.now().atStartOfDay().plusSeconds((long) Math.pow(10, 9)), gigasecond.getDateTime());
    }

    @Test
    public void testNullDatTime() {
        Gigasecond gigasecond = new Gigasecond((LocalDateTime) null);
        assertEquals(LocalDateTime.now().plusSeconds((long) Math.pow(10, 9)).withNano(0),
